...
const RESET_REQUEST_COOLDOWN_MS = 60 * 1000; // per account, so an unauthenticated caller can't spam paid SMS
...
export async function requestPasswordReset(phoneRaw: string) {
  ...
  // Silent on purpose — the caller sees the same response either way, so this
  // never reveals whether the number is registered or whether a code was just sent.
